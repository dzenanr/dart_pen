...
$$.Pen = {"":
    ...
    for (var t1 = $.iterator(commandList); t1.hasNext$0() === true; ) {
      ...
      if ($.gtB($.get$length(command), 0)) {
        switch ($.index(command, 0)) {
          case 'down':
            var downString = $.index(command, 1);
            if ($.eqB(downString, 'true')) this.set$down(true);
            else {
              $.eqB(downString, 'false') && this.set$down(false);
            }
            break;
          case 'color':
            this.set$color($.index(command, 1));
            break;
          case 'randomColor':
            this.randomColor$0();
            break;
          case 'width':
            this.set$width($.parseInt($.index(command, 1)));
            break;
          case 'randomWidth':
            this.randomWidth$0();
            break;
          case 'write':
            this.set$write($.index(command, 1));
            break;
          case 'moveTo':
            this.moveTo$1($.Point($.parseDouble($.index(command, 1)), $.parseDouble($.index(command, 2))));
            break;
          case 'moveToStart':
            this.moveToStart$0();
            break;
          case 'randomMoveTo':
            ...
            break;
          case 'move':
            if ($.eqB($.get$length(command), 2)) this.move$1($.parseDouble($.index(command, 1)));
            else {
              if ($.eqB($.get$length(command), 3)) this.move$2($.parseDouble($.index(command, 1)), $.parseDouble($.index(command, 2)));
              else {
                $.eqB($.get$length(command), 4) && this.move$3($.parseDouble($.index(command, 1)), $.parseDouble($.index(command, 2)), $.parseInt($.index(command, 3)));
              }
            }
            break;
          case 'right':
            this.right$1($.parseDouble($.index(command, 1)));
            break;
          case 'left':
            this.left$1($.parseDouble($.index(command, 1)));
            break;
          case 'forward':
            this.forward$1($.parseDouble($.index(command, 1)));
            break;
          case 'backward':
            this.backward$1($.parseDouble($.index(command, 1)));
            break;
          case 'randomMove':
            this.randomMove$0();
            break;
          case 'all':
            this.all$1($.parseInt($.index(command, 1)));
            break;
          case 'randomAll':
            this.randomAll$0();
        }
 ...
 fromCommands$0: function() {
  for (var t1 = $.iterator(this.commands), result = ''; t1.hasNext$0() === true; ) {
    ...
    if (t2.length > 1) {
      ...
      var t4 = t2[0];
      if (typeof t4 !== 'string') return this.fromCommands$0$bailout(2, result, t2, t1, t4);
      if (!(t4 === '')) {
        for (var commandLine = t4, i = 1; i < t2.length; ++i) {
          t3 = commandLine + ', ';
          t4 = t2.length;
          if (i < 0 || i >= t4) throw $.ioore(i);
          commandLine = t3 + $.S(t2[i]);
        }
 ...
 fromCommands$0$bailout: function(state, env0, env1, env2, env3) {
  switch (state) {
    case 1:
      result = env0;
      t1 = env1;
              ...
              switch (state) {
                ...
                case 2:
                  ...
                  if (!$.eqB(commandLine, '')) {
                    var i = 1;
                    L1: while (true) {
                      if (!$.ltB(i, $.get$length(t2))) break L1;
                      commandLine = $.S(commandLine) + ', ' + $.S($.index(t2, i));
                      ++i;
                    }
 ...
 all$1: function(repeat) {
  ...
  $.add$1(this.commands, ['all', repeat]);
 },
 all$1$bailout: function(repeat, state, env0) {
  switch (state) {
    case 1:
      repeat = env0;
      break;
 ...
 randomMove$0: function() {
  var t1 = $.randomSign();
  if (typeof t1 !== 'number') return this.randomMove$0$bailout(1, t1, 0, 0);
  var t2 = $.randomNum(360);
  if (typeof t2 !== 'number') return this.randomMove$0$bailout(2, t1, t2, 0);
  t2 = t1 * t2;
  t1 = $.randomSign();
 ...
 move$3: function(turn, advance, repeat) {
  if (typeof advance !== 'number') return this.move$3$bailout(turn, advance, repeat, 1, advance);
  ...
  if ($.eqB(repeat, 0)) {
    if (advance === 0) $.add$1(this.commands, ['move', turn]);
    else $.add$1(this.commands, ['move', turn, advance]);
  } else $.add$1(this.commands, ['move', turn, advance, repeat]);
 },
 move$3$bailout: function(turn, advance, repeat, state, env0) {
  switch (state) {
    case 1:
      advance = env0;
      break;
  ...
  switch (state) {
    ...
    case 1:
      ...
      if ($.gtB(lineCount, 0)) {
        ...
        var i = 0;
        L0: while (true) {
          if (!$.ltB(i, segment.lineCount)) break L0;
          var line = $.Line$next$1(lastLine);
          $.indexSet(segment.lines, i, line);
          line.set$angle(turn);
          line.set$pixels(advance);
      ...
      if ($.eqB(repeat, 0)) {
        if ($.eqB(advance, 0)) $.add$1(this.commands, ['move', turn]);
        else $.add$1(this.commands, ['move', turn, advance]);
      } else $.add$1(this.commands, ['move', turn, advance, repeat]);
  }
 },
 ...
 randomMoveTo$0: function() {
  var t1 = $.randomSign();
  if (typeof t1 !== 'number') return this.randomMoveTo$0$bailout(1, t1, 0, 0);
  var t2 = this.start.get$x();
  if (typeof t2 !== 'number') return this.randomMoveTo$0$bailout(2, t1, t2, 0);
  t2 = t1 * t2;
  t1 = $.randomSign();
  if (typeof t1 !== 'number') return this.randomMoveTo$0$bailout(3, t2, t1, 0);
  var t3 = this.start.get$y();
  if (typeof t3 !== 'number') return this.randomMoveTo$0$bailout(4, t2, t1, t3);
  return this.moveTo$1($.randomPoint(t2, t1 * t3));
 },
 randomMoveTo$0$bailout: function(state, env0, env1, env2) {
  switch (state) {
    case 1:
      t1 = env0;
      break;
 ...
 moveTo$1: function(point) {
  ...
  $.add$1(this.path.get$segments(), segment);
  var line = $.Line$next$1(lastLine);
  var t1 = segment.lines;
  if (typeof t1 !== 'object' || t1.constructor !== Array || !!t1.immutable$list || !!!t1.is$JavaScriptIndexingBehavior) return this.moveTo$1$bailout(point, 1, line, t1);
  var t2 = t1.length;
  if (0 >= t2) throw $.ioore(0);
  t1[0] = line;
  line.endPoint = point;
  $.add$1(this.commands, ['moveTo', point.get$x(), point.get$y()]);
 },
 moveTo$1$bailout: function(point, state, env0, env1) {
  switch (state) {
    case 1:
      line = env0;
      t1 = env1;
 ...
 randomColor$0: function() {
  var t1 = $.randomListElement($.colorList);
  this.set$color(t1);
  return t1;
 },
 get$write: function() {
 ...
 set$write: function(write) {
  this._write = write;
  $.add$1(this.commands, ['write', this._write]);
 },
 get$width: function() {
  return this._width;
 },
 set$width: function(width) {
  ...
  $.add$1(this.commands, ['width', this._width]);
 },
 get$color: function() {
  return this._color;
 },
 set$color: function(color) {
  this._color = color;
  $.add$1(this.commands, ['color', this._color]);
 },
 get$down: function() {
  return this._down;
 },
 set$down: function(down) {
  this._down = down;
  $.add$1(this.commands, ['down', this._down]);
 },
 erase$0: function() {
  this._init$0();
 },
...
$$.Path = {"":
 ...
 toString$0: function() {
  ...
  for (var t1 = $.iterator(this.segments), lineCount = 0; t1.hasNext$0() === true; ) {
    lineCount = $.add(lineCount, $.get$length(t1.next$0().get$lines()));
  }
 ...
 double$0: function() {
  try {
    var copiedSegments0 = $.List((void 0));
    $.setRuntimeTypeInfo(copiedSegments0, ({E: 'Segment'}));
    var copiedSegments = copiedSegments0;
    var last = this.lastLine$0();
    for (i = 0; $.ltB(i, $.get$length(this.segments)); i = $.add(i, 1)) {
      var segment = $.index(this.segments, i).copy$1(last);
      $.add$1(copiedSegments, segment);
      last = segment.lastLine$0();
    }
    for (var t1 = $.iterator(copiedSegments); t1.hasNext$0() === true; ) {
      var copiedSegment = t1.next$0();
      $.add$1(this.segments, copiedSegment);
    }
  } catch (exception) {
    t1 = $.unwrapException(exception);
    var error = t1;
    $.print('Error in doubling the path! -- ' + $.S(error));
  }
  var i;
 },
 lastLine$0: function() {
  var last = (void 0);
  try {
    last = $.last(this.segments).lastLine$0();
  } catch (exception) {
    var t1 = $.unwrapException(exception);
    var error = t1;
    $.print('Error in finding the last line of the path! -- ' + $.S(error));
  }
  return last;
 },
 Path$0: function() {
...
$$.Segment = {"":
 ...
 copy$1: function(last) {
  ...
  var previousLine = last;
  var i = 0;
  while (true) {
    var t1 = this.lineCount;
    if (typeof t1 !== 'number') return this.copy$1$bailout(last, 1, i, previousLine, segment, t1);
    if (!(i < t1)) break;
    t1 = this.lines;
    if (typeof t1 !== 'string' && (typeof t1 !== 'object' || t1.constructor !== Array || !!!t1.is$JavaScriptIndexingBehavior)) return this.copy$1$bailout(last, 2, i, previousLine, segment, t1);
    var t2 = t1.length;
    if (i < 0 || i >= t2) throw $.ioore(i);
    var line = t1[i].copy$1(previousLine);
    t1 = segment.lines;
    if (typeof t1 !== 'object' || t1.constructor !== Array || !!t1.immutable$list || !!!t1.is$JavaScriptIndexingBehavior) return this.copy$1$bailout(last, 3, line, i, segment, t1);
    var t3 = t1.length;
 ...
 copy$1$bailout: function(last, state, env0, env1, env2, env3) {
  switch (state) {
    case 1:
      i = env0;
      previousLine = env1;
    ...
    case 3:
      L0: while (true) {
        switch (state) {
          ...
          case 2:
            state = 0;
            var line = $.index(t1, i).copy$1(previousLine);
            t1 = segment.lines;
          case 3:
            state = 0;
            $.indexSet(t1, i, line);
 ...
 lastLine$0: function() {
  var last = (void 0);
  try {
    last = $.last(this.lines);
  } catch (exception) {
    var t1 = $.unwrapException(exception);
    var error = t1;
    $.print('Error in finding the last line of the segment! -- ' + $.S(error));
  }
  return last;
 },
...
$$.Line = {"":
 ["endPoint?", "_pixels", "_angle", "cumulativeAngle?", "beginPoint?", "lastLine"],
 super: "Object",
 copy$1: function(previousLine) {
  var line = $.Line$next$1(previousLine);
  line.beginPoint = previousLine.get$endPoint();
  line.set$angle(this.get$angle());
  line.set$pixels(this.get$pixels());
  return line;
 },
 findEndPoint$3: function(startPoint, startAngle, length$) {
  var x1 = startPoint.get$x();
  var y1 = startPoint.get$y();
  var radian = $.div($.mul(startAngle, 3.141592653589793), 180);
  return $.Point($.add(x1, $.mul(length$, $.cos(radian))), $.add(y1, $.mul(length$, $.sin(radian))));
 },
 get$pixels: function() {
  return this._pixels;
 },
 set$pixels: function(pixels) {
  this._pixels = pixels;
  this.endPoint = this.findEndPoint$3(this.beginPoint, this.cumulativeAngle, pixels);
 },
 get$angle: function() {
  return this._angle;
 },
 set$angle: function(angle) {
  this._angle = angle;
  if ($.eqNullB(this.lastLine)) this.cumulativeAngle = angle;
  else {
    this.cumulativeAngle = $.add(this.lastLine.get$cumulativeAngle(), angle);
    if ($.gtB(this.cumulativeAngle, 360)) this.cumulativeAngle = $.sub(this.cumulativeAngle, 360);
  }
  this.endPoint = this.findEndPoint$3(this.beginPoint, this.cumulativeAngle, this.get$pixels());
 },
 lastLine$0: function() { return this.lastLine.$call$0(); },
...
$$.Input = {"":
 ...
 Input$1: function(pen) {
  ...
  $.add$1(this.downCheckbox.get$on().get$change(), new $.Closure13(this));
  this.colorSelect = $.document().query$1('#color');
  $.add$1(this.colorSelect.get$on().get$change(), new $.Closure14(this));
...
$$.Commands = {"":
 ["commandsTextArea?", "drawButton", "hideButton", "showButton", "pen?"],
 super: "Object",
 clear$0: function() {
 ...
 Commands$1: function(pen) {
  ...
  $.add$1(this.showButton.get$on().get$click(), new $.Closure10(this));
  this.hideButton = $.document().query$1('#hide');
  $.add$1(this.hideButton.get$on().get$click(), new $.Closure11(this));
  this.drawButton = $.document().query$1('#draw');
  $.add$1(this.drawButton.get$on().get$click(), new $.Closure12(this));
  this.commandsTextArea = $.document().query$1('#commands');
 }
};

$$.Data = {"":
 ["pathTextArea?", "clearButton", "displayButton", "pen?"],
 super: "Object",
 clear$0: function() {
...
$$._DOMWindowCrossFrameImpl = {"":
 ...
 postMessage$3: function(message, targetOrigin, messagePorts) {
  if ($.eqNullB(messagePorts)) $._postMessage2(this._window, message, targetOrigin);
  else $._postMessage3(this._window, message, targetOrigin, messagePorts);
 },
...
$.stringReplaceAllUnchecked = function(receiver, from, to) {
  ...
  if (typeof from === 'string') {
    ...
    return $.stringReplaceJS(receiver, $.regExpMakeNative($.JSSyntaxRegExp$3((from.replace($.regExpMakeNative($.CTC5, true), "\\$&")), false, false), true), to);
  }
...
$.Data$1 = function(pen) {
  var t1 = new $.Data((void 0), (void 0), (void 0), pen);
  t1.Data$1(pen);
  return t1;
};
...
$.colorMap = function() {
  $.colorList = ['black', 'blue', 'brown', 'gray', 'green', 'orange', 'red', 'yellow'];
...
$.randomListElement = function(list) {
  return $.index(list, $.randomInt($.sub($.get$length(list), 1)));
};
...
$.Input$1 = function(pen) {
  var t1 = new $.Input((void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), (void 0), pen);
  t1.Input$1(pen);
  return t1;
};
